...
export async function buildPostcard(env: Env, caricature: R2ObjectBody, watermarkKey: string | null) {
  ...
  if (watermarkKey) {
    ...
    if (watermark) {
      pipeline = pipeline.draw(env.IMAGES.input(watermark.body).transform({ width: 540 }), { bottom: 56, right: 56, opacity: 0.95 });
    }
  }
