...
describe('admin event watermark', () => {
  ...
  it('compiles the editor and propagates the selected configuration to the workflow', async () => {
    ...
    expect(editor).toContain('xl:grid-cols-[minmax(0,3fr)_minmax(20rem,2fr)]');
    ...
    expect(editor).toContain('>Left watermark</h3>\n              <button class:list=');
    expect(editor).toContain('>Right watermark</h3>\n              <button class:list=');
    expect(editor).toContain('data-watermark-side="left"');
    expect(editor).toContain('data-watermark-side="right"');
    expect(editor).toContain('id="save-watermarks" type="button" disabled>Save</button>');
